...
class SimulatorEngine:
    ...
    def run(self, decisions: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for s in self.config.get("symbols", []):
            ...
            tf = s.get("timeframe", "")
            if sym:
                symbol_configs[sym] = s
            if sym and tf:
                symbol_configs[sym + "_" + tf] = s

        self.latest_intents = build_order_intents(decisions, symbol_configs, initial_cash, risk_config)
        ...
        for s in self.config.get("symbols", []):
            key1 = s.get("symbol", "")
            key2 = s.get("symbol", "") + "_" + s.get("timeframe", "")
            symbol_configs[key1] = s
            symbol_configs[key2] = s
