#!/usr/bin/env python3
"""
CLI tool to run walk-forward validation on synthetic or CSV data.
...
def _synthetic_data(symbols: list, bars: int = 200) -> dict:
    import random
    random.seed(42)
    data = {}
    for sym in symbols:
...
def main():
    ...
    print("PAPER-ONLY DISCLAIMER: Walk-forward results are for research only. No live trading.")
    ...
    wf = WalkForwardValidation(data, factory, n_folds=args.n_folds)
    results = wf.run()
    out = {"strategy": args.strategy, "folds": results}
    if args.output:
